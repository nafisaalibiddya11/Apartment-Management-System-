...
namespace ApartmentManagement.Applications
{
    public partial class AdminProfile : Form
    {
        ...
        {
            this.Ds = UserRepository.GetManagerDataSet();
            this.dgvmanager.AutoGenerateColumns = false;
            this.dgvmanager.DataSource = this.Ds.Tables[0];
        }
        public void PopulateGridViewManager()
        {
            SetManagers();
        }
        public void UpdatePopulateGridViewManager(string text)
        {
            this.Ds = UserRepository.GetManagerDataSetChanged(text);
            this.dgvmanager.AutoGenerateColumns = false;
            this.dgvmanager.DataSource = this.Ds.Tables[0];
        }
        ...
        {
            string id = this.dgvmanager.CurrentRow.Cells["managerid"].Value.ToString();
        ...
        private void txtmanagersearch_TextChanged(object sender, EventArgs e)
        {
            this.UpdatePopulateGridViewManager(this.txtmanagersearch.Text);
        }
        ...
        {
            this.Visible = false;
            request set = SetManagers;
            AddManager am = new AddManager(this,set);
            am.Visible = true;
        }
